...
void* threadfunc(void* thread_param)
{
    ...
    int got_mutex = pthread_mutex_lock(thread_func_args->mutex);
    if (0 == got_mutex)
    {
        usleep(1000 * thread_func_args->wait_to_release_ms);
        int released_mutex = pthread_mutex_unlock(thread_func_args->mutex);
        thread_func_args->thread_complete_success = (0 == released_mutex);
    }
...
bool start_thread_obtaining_mutex(pthread_t* thread, pthread_mutex_t* mutex, int wait_to_obtain_ms, int wait_to_release_ms)
{
    ...
    int retval = pthread_create(thread, NULL, threadfunc, thread_data);
